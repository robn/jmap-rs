...
impl fmt::Display for ParseError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            ...
            "{}",
            match *self {
                ParseError::InvalidJsonType(ref e) =>
                    format!("invalid JSON type for conversion to {}", e),
                ParseError::InvalidStructure(ref e) =>
                    format!("invalid value structure for conversion to {}", e),
...
impl FromJson for i64 {
    fn from_json(json: &Json) -> Result<i64, ParseError> {
        match *json {
            ...
            Json::I64(n) if n >= i64::MIN && n <= i64::MAX => Ok(n as i64),
...
impl FromJson for i32 {
    fn from_json(json: &Json) -> Result<i32, ParseError> {
        match *json {
            Json::U64(n) if n <= (i32::MAX as u64) => Ok(n as i32),
            Json::I64(n) if n >= (i32::MIN as i64) && n <= (i32::MAX as i64) => Ok(n as i32),
...
impl FromJson for usize {
    fn from_json(json: &Json) -> Result<usize, ParseError> {
        match *json {
            Json::U64(n) if n <= (usize::MAX as u64) => Ok(n as usize),
            Json::I64(n) if n >= (usize::MIN as i64) && n <= (usize::MAX as i64) => Ok(n as usize),
    ...
    T: FromJson,
{
    fn from_json(json: &Json) -> Result<Vec<T>, ParseError> {
        match *json {
            Json::Array(ref a) => {
                let (ok, mut err): (Vec<_>, Vec<_>) = a
                    .iter()
                    .map(|ref j| T::from_json(j))
...
impl<T> FromJsonField for Option<T>
where
    T: FromJson,
{
    fn from_json_field(json: &BTreeMap<String, Json>, field: &str) -> Result<Self, ParseError> {
...
impl<T> FromJsonField for Presence<T>
where
    T: FromJson,
{
    fn from_json_field(json: &BTreeMap<String, Json>, field: &str) -> Result<Self, ParseError> {
...
impl<T> FromJsonField for Presence<Option<T>>
where
    T: FromJson,
{
    fn from_json_field(json: &BTreeMap<String, Json>, field: &str) -> Result<Self, ParseError> {
...
impl<T> ToJsonField for Presence<T>
where
    T: ToJson,
{
    fn to_json_field(&self, json: &mut BTreeMap<String, Json>, field: &str) {
